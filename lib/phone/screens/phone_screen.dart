    ...
    return Scaffold(
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => const PhoneSettingsScreen(),
                ),
              );
            },
          ),
      ...
      body: Container(
        ...
        child: PageView(
          ...
          children: const [RecentCallsScreen(), ContactsPage()],
        ),
      ),
    );
  }
    ...
    return Container(
      ...
      child: FittedBox(
        child: Row(
          ...
          children: [_buildTab('Phone', 0), _buildTab('Contacts', 1)],
        ),
      ),
    );
  }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: callLogProvider.isLoading
            ...
            : RefreshIndicator(
                onRefresh: () => Provider.of<CallLogProvider>(
                  context,
                  listen: false,
                ).syncDeviceLogsToDb(),
...
class _ContactsPageState extends State<ContactsPage>
  ...
  List<dynamic> _buildDisplayList(List<fc.Contact> allContacts) {
    final filteredContacts = allContacts.where((contact) {
      final nameMatches = contact.displayName.toLowerCase().contains(
        _searchQuery.toLowerCase(),
      );
      final numberMatches = contact.phones.any(
        (p) => p.number.contains(_searchQuery),
      );
          ...
          floatingActionButton: FloatingActionButton(
            onPressed: () async {
              final result = await Navigator.push<bool>(
                context,
                MaterialPageRoute(
                  builder: (context) => const SaveContactScreen(),
                ),
              );
...
class _DialpadSheetState extends State<DialpadSheet> {
  ...
  List<Widget> _getDialButtons() {
    ...
    return numbers
        .map((e) => _buildDialButton(e['number']!, e['letters']!))
        .toList();
  }
